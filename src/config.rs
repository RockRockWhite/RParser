...
impl Config {
    pub fn parse_config(path: &str) -> Result<Config, Box<dyn Error>> {
        // 读取文件
        let (declarations, definitions, rules, variables) = Self::read_config_file(path)?;
        // 将definitions中的变量提取出来
        let definitions = Self::parse_definations(&definitions)?;
        // 将rules中的变量提取出来
    ...
    fn parse_definations(definitions: &str) -> Result<HashMap<String, String>, Box<dyn Error>> {
        ...
        for (key, value) in &mut raw_definations {
            res.insert(
                key.clone(),
                Self::replace_regex_variables(value, &res)?.clone(),
            );
        }
